...
class MethodOptions:
    ...
    trust_region_rho_ub: float = 0.99
    ...
    def __post_init__(self):
        ...
        if self.score_test_hdf5_file_name is not None:
            if self.score_test_hdf5_trait_name is None:
                raise ValueError(
                    "score_test_hdf5_trait_name must be specified if score_test_hdf5_file_name is specified."
                )
            self.score_test_hdf5_trait_name = self.score_test_hdf5_trait_name.replace(
                "/", "_"
            )
            if not self.use_surrogate_markers:
                raise ValueError(
                    "use_surrogate_markers must be True if score_test_hdf5_file_name is specified."
                )
            if self.match_by_position:
                raise ValueError(
                    "match_by_position must be False if score_test_hdf5_file_name is specified."
                )


def _filter_binary_annotations(
    annotation_data: pl.DataFrame, annotation_columns: List[str], verbose: bool = False
    ...
    if verbose:
        print(
            f"Binary filtering: Discarding {len(cols_to_drop)} non-binary/non-Boolean annotations: {cols_to_drop}"
        )
...
class GraphREML(ParallelProcessor):
    ...
    def _initialize_block_zscores(
        ...
        if verbose:
            print(
                f"{len(annot_df)} sumstats variants before merging, {len(ldgm.variant_info)} afterward"
            )
            ...
            if surrogate_row is None:
                ...
                if surrogate_row is None:
                    ...
                    surrogate_row = _surrogate_marker(
                        ldgm, idx, variant_info_nonmissing
                    )
        ...
        first_index_mask = (
            variant_info.select(pl.col("index").is_first_distinct())
            .to_numpy()
            .flatten()
        )
    ...
    @staticmethod
    def _load_block_surrogate_map(
        surrogate_markers_path: str, block_name: Optional[str]
    ) -> Optional[np.ndarray]:
        ...
        with h5py.File(surrogate_markers_path, "r") as h5:
            if block_name in h5:
                ...
                raise ValueError(
                    f"Block {block_name} not found in {surrogate_markers_path}"
                )
    ...
    def _compute_variant_grad(
        ...
        """
        many_samples = 200
        grad = gaussian_likelihood_gradient(
            Pz, ldgm, n_samples=many_samples, del_M_del_a=None, seed=seed
        )
    ...
    @staticmethod
    def _compute_variant_hessian_diag(
        ldgm: PrecisionOperator,
        Pz: np.ndarray,
        ...
        first_term = gaussian_likelihood_hessian(
            Pz,
            ldgm,
            del_M_del_a=None,
        ...
        second_term = (
            del_L_del_xi.ravel() / del_h2i_del_xi_indexed * del2_h2i_del_xi2_indexed
        )
    ...
    @classmethod
    def _get_variant_jackknife_assignments(
        cls, block_indptrs: List[int], num_groups: int
    ) -> np.ndarray:
        """Get group assignment for each variant."""
        num_blocks = len(block_indptrs) - 1
        group_sizes = cls._group_blocks(
            np.ones(num_blocks, dtype=int), num_groups
        ).astype(int)
    ...
    def _get_groups(cls, num_blocks: int, num_groups: int) -> np.ndarray:
        ...
        sizes = cls._group_blocks(np.ones(num_blocks, dtype=int), num_groups).astype(
            int
        )
    ...
    @staticmethod
    def _compute_pseudojackknife(
        gradient_blocks: np.ndarray, hessian_blocks: np.ndarray, params: np.ndarray
    ) -> np.ndarray:
        ...
        gradient = gradient_blocks.sum(
            axis=0
        )  # Shape: (num_params,) - sum across blocks
        hessian = hessian_blocks.sum(
            axis=0
        )  # Shape: (num_params, num_params) - sum across blocks
    ...
    @staticmethod
    def _compute_jackknife_heritability(
        block_data: list, jackknife_params: np.ndarray, model: ModelOptions
    ) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for block in range(num_blocks):
            if len(block_data[block]["sumstats"]) == 0:
                continue
            annotations = (
                block_data[block]["sumstats"]
                .select(model.annotation_columns)
                .to_numpy()
            )
        ...
        for jk in range(num_jk):
            ...
            for annotations in block_annotations:
                ...
                jackknife_h2[jk, :] += (per_snp_h2[:, np.newaxis] * annotations).sum(
                    axis=0
                )
    ...
    def _wald_log10pvalue(jackknife_estimates: np.ndarray) -> float:
        ...
        standard_error = np.sqrt(
            (n_blocks - 1) * np.var(jackknife_estimates, axis=0, ddof=1)
        )
        # Calculate two-sided log10(p-value)
        log10pval = (
            np.log(2) + sps.norm.logcdf(-np.abs(point_estimate / standard_error))
        ) / np.log(10)
        return log10pval
